...
<head>
    ...
    <title>Data Formulir Pendaftaran</title>
...
<body>
    ...
    <div class="container mt-5">
        <h2 class="text-center">Data Formulir Pendaftaran</h2>
        
        <!-- Form Pencarian -->
        <div class="row mb-4">
            <div class="col-md-6 mx-auto">
                <form method="GET" action="">
                    <input type="hidden" name="page" value="delete_form_regis">
                    <div class="input-group">
                        <input type="text" class="form-control" name="search" placeholder="Cari berdasarkan nama file..." value="<?php echo isset($_GET['search']) ? htmlspecialchars($_GET['search']) : ''; ?>">
        ...
        <div class="table-container">
            ...
            if (isset($_GET['delete_id'])) {
                ...
                $sql = "DELETE FROM formulir_pendaftaran WHERE id = ?";
                ...
                if ($stmt->execute()) {
                    ...
                    echo "<script>setTimeout(function() { window.location.href = 'admin.php?page=delete_form_regis'; }, 2000);</script>";
                } else {
            ...
            // Query untuk menampilkan data dari tabel formulir_pendaftaran
            $search = isset($_GET['search']) ? '%' . $conn->real_escape_string($_GET['search']) . '%' : '';
            if (!empty($search)) {
                $sql = "SELECT id, file_name, file_path FROM formulir_pendaftaran WHERE file_name LIKE ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("s", $search);
                $stmt->execute();
                $result = $stmt->get_result();
            } else {
                $sql = "SELECT id, file_name, file_path FROM formulir_pendaftaran";
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>
                            ...
                            <td>{$row['file_path']}</td>
                            <td>
                                <a href='admin.php?page=delete_form_regis&delete_id={$row['id']}' class='btn btn-danger btn-sm' onclick='return confirm(\"Apakah Anda yakin ingin menghapus data ini?\")'>Hapus</a>
                            </td>
                          </tr>";
                }
