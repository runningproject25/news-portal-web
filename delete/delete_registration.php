    ...
    <div class="container-xxl mt-5">
        ...
        <div class="row mb-4">
            <div class="col-md-6 mx-auto">
                <form method="GET" action="">
                    <input type="hidden" name="page" value="delete_registration">
                    <div class="input-group">
                        <input type="text" class="form-control" name="search" placeholder="Cari berdasarkan title..." value="<?php echo isset($_GET['search']) ? htmlspecialchars($_GET['search']) : ''; ?>">
        ...
        <div class="table-container">
            ...
            if (isset($_GET['delete_id'])) {
                ...
                $sql = "DELETE FROM regis WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param('i', $id);
                
                if ($stmt->execute()) {
                    echo "<div class='alert alert-success alert-top'>Data berhasil dihapus.</div>";
                    echo "<script>setTimeout(function() { window.location.href = 'admin.php?page=delete_registration'; }, 2000);</script>";
                } else {
            ...
            // Query untuk menampilkan data dari tabel regis dengan pencarian
            $search = isset($_GET['search']) ? '%' . $conn->real_escape_string($_GET['search']) . '%' : '';
            if (!empty($search)) {
                $sql = "SELECT id, title, image, content, created_at FROM regis WHERE title LIKE ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("s", $search);
                $stmt->execute();
                $result = $stmt->get_result();
            } else {
                $sql = "SELECT id, title, image, content, created_at FROM regis";
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>
                            ...
                            <td>
                                <a href='admin.php?page=delete_registration&delete_id={$row['id']}' class='btn btn-danger btn-sm' onclick='return confirm(\"Apakah Anda yakin ingin menghapus data ini?\")'>Hapus</a>
                            </td>
                          </tr>";
                }
