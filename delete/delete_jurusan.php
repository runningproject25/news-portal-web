        ...
        <div class="row mb-4">
            <div class="col-md-6 mx-auto">
                <form method="GET" action="">
                    <input type="hidden" name="page" value="delete_jurusan">
                    <div class="input-group">
                        <input type="text" class="form-control" name="search" placeholder="Cari berdasarkan jurusan, program studi, atau akreditasi..." value="<?php echo isset($_GET['search']) ? htmlspecialchars($_GET['search']) : ''; ?>">
        ...
        <div class="table-container">
            ...
            if (isset($_GET['delete_id'])) {
                ...
                if ($stmt->execute()) {
                    ...
                    echo "<script>setTimeout(function() { window.location.href = 'admin.php?page=delete_jurusan'; }, 2000);</script>";
                } else {
            ...
            if (!empty($search)) {
                $sql = "SELECT id, jurusan, program_studi, akreditasi, daya_tampung FROM jurusan WHERE jurusan LIKE ? OR program_studi LIKE ? OR akreditasi LIKE ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sss", $search, $search, $search);
                $stmt->execute();
                $result = $stmt->get_result();
            } else {
                $sql = "SELECT id, jurusan, program_studi, akreditasi, daya_tampung FROM jurusan";
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>
                            ...
                            <td>
                                <a href='admin.php?page=delete_jurusan&delete_id={$row['id']}' class='btn btn-danger btn-sm' onclick='return confirm(\"Apakah Anda yakin ingin menghapus data ini?\")'>Hapus</a>
                            </td>
                          </tr>";
                }
