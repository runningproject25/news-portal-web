...
<body>
    ...
    <div class="container mt-5">
        ...
        // Validasi ID registration
        if (isset($_GET['id']) && filter_var($_GET['id'], FILTER_VALIDATE_INT)) {
            $id = $_GET['id'];

            // Ambil data registration berdasarkan ID
            $stmt = $conn->prepare("SELECT title, content, image FROM regis WHERE id = ?");
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $stmt = $conn->prepare("UPDATE regis SET title = ?, content = ?, image = ?, created_at = ? WHERE id = ?");
                $stmt->bind_param("ssssi", $title, $content, $image, $updated_at, $id);

                if ($stmt->execute()) {
                    header('Location: admin.php?page=edit_registration'); // Redirect ke halaman edit_registration
                    exit();
                } else {
        ...
        <form action="admin.php?page=edit_registration_process&id=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
            <div class="mb-3">
